...
async def get_graph_data(q: Optional[str] = Query(None)):
    ...
    try:
        ...
        for result in results_list:
            ...
            category = metadata.get("category", "").lower() if isinstance(metadata, dict) else ""
        ...
        for node in nodes:
            if node.id != "center":
                link = GraphLink(
                    source="center", 
                    target=node.id,
                    value=page_scores.get(node.id, 0.5)
                )
                links.append(link)
        ...
        for i, node1 in enumerate(node_list):
            for node2 in node_list[i+1:]:
                ...
                if score1 > 0.7 and score2 > 0.7:
                    ...
                    links.append(link)
                    logger.debug(f"   🔗 Linked: {node1.name[:20]} <-> {node2.name[:20]} (strength: {link_strength:.2f})")
...
@router.get("/graph/expand/{node_id}", response_model=GraphResponse)
async def expand_node(node_id: str, limit: int = Query(5, ge=1, le=20)):
    ...
    try:
        ...
        logger.info(f"✅ Expanded node {node_id}: +{len(nodes)} nodes, +{len(links)} links")
