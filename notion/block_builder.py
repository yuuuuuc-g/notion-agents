...
def parse_rich_text(text: str) -> List[Dict]:
    ...
    for part in parts:
        ...
        elif part.startswith("`"): add_text(part[1:-1], {"code": True})
        elif part.startswith("[") and "](" in part:
            try:
                idx = part.rindex("](")
                add_text(part[1:idx], url=part[idx+2:-1].strip())
            except: add_text(part)
        elif part.startswith("**"): add_text(part[2:-2], {"bold": True})
        elif part.startswith("*"): add_text(part[1:-1], {"italic": True})
        elif part.startswith("~"): add_text(part[1:-1], {"strikethrough": True})
        else: add_text(part)
...
def markdown_to_blocks(markdown_text: str) -> List[Dict]:
    ...
    root_blocks = []
    stack = [{"children": root_blocks, "indent": -1}]
    
    # 状态机变量
    code_mode, math_mode = False, False
    code_content, math_content, table_rows = [], [], []
    ...
    for line in lines:
        ...
        if stripped.startswith("```"):
            if code_mode:
                ...
                stack[-1]["children"].append({
                    ...
                    "code": {"rich_text": [{"type": "text", "text": {"content": content[:2000]}}], "language": "plain text"}
                })
                code_mode = False; code_content = []
            else: code_mode = True; continue
        ...
        if stripped.startswith("|"):
            ...
            if not all(re.match(r"^[-: ]+$", c) for c in cells if c): table_rows.append(cells)
            continue
        elif table_rows:
            t = _flush_table(table_rows)
            if t: stack[-1]["children"].append(t)
            table_rows = []
        ...
        new_block, is_container = None, False
        
        # 3. 标题识别与自动降级 (解决 #### 问题)
        if stripped.startswith("# "):
            new_block = {"type": "heading_1", "heading_1": {"rich_text": parse_rich_text(stripped[2:])}}
        elif stripped.startswith("## "):
            new_block = {"type": "heading_2", "heading_2": {"rich_text": parse_rich_text(stripped[3:])}}
        elif stripped.startswith("### ") or stripped.startswith("#### "):
            new_block = {"type": "heading_3", "heading_3": {"rich_text": parse_rich_text(re.sub(r"^#+\s+", "", stripped))}}
        
        # 4. 列表与引用
        elif re.match(r"^[-*]\s+", stripped):
            new_block = {"type": "bulleted_list_item", "bulleted_list_item": {"rich_text": parse_rich_text(stripped[2:])}}
            is_container = True
        elif stripped.startswith("> "):
            new_block = {"type": "quote", "quote": {"rich_text": parse_rich_text(stripped[2:])}}
            is_container = True
        else:
            new_block = {"type": "paragraph", "paragraph": {"rich_text": parse_rich_text(stripped)}}
    ...
    def _clean(blocks):
        for b in blocks:
            if "children" in b:
                if not b["children"]: del b["children"]
                else: _clean(b["children"])
    _clean(root_blocks)
    return root_blocks